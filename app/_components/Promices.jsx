...
function Promices() {
  ...
  return (
    <div className="h-[200px] flex items-center justify-evenly ">
      {services.map((item, i) => {
        return (
          <div key={i} className="flex flex-col md:flex-row items-center gap-[10px]">
            {item.image}
            <div className="flex flex-col justify-between items-center md:items-start py-[5px]">
              <h2 className="text-[12px] md:text-[25px] font-bold">{item.heading}</h2>
              <p className="text-[8px] md:text-[16px] font-semibold text-slate-500">{item.para}</p>
            </div>
          </div>
        );
      })}
