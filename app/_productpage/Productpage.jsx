...
async function Productpage({ category, subcat, productid, color }) {
  ...
  const priceBeforeDiscount = filteredProduct.discount > 0
    ? Math.floor((filteredProduct.price / (100 - filteredProduct.discount)) * 100)
    : null;
    ...
    <article>
      <header className="flex flex-col lg:flex-row items-start p-[10px]">
        <div className="w-full lg:w-[50%] lg:sticky lg:top-[130px]">
          <Imagescomp filteredproducts={filteredProduct} color={color} token={token} />
        </div>
        <section className="w-full lg:w-[50%] p-[10px] px-[20px]">
          <Breadcrumbs category={category} subcat={subcat} productName={filteredProduct.name} />
          ...
          <PriceDisplay filteredProduct={filteredProduct} priceBeforeDiscount={priceBeforeDiscount} />
      ...
      <ProductCare />
      <Promices />
      <Similarproducts allproducts={allproducts} category={category} subcat={subcat} productid={productid} />
    </article>
  );
}

const Breadcrumbs = ({ category, subcat, productName }) => (
  <nav className="sticky top-[60px] lg:top-[110px] bg-white flex gap-[10px] text-[#87878] text-[14px] z-20 whitespace-nowrap cursor-pointer">
    <Link className="lg:hover:text-cyan-500" href="/">Home</Link>
    <span>{">"}</span>
    <Link className="lg:hover:text-cyan-500" href={`/${category}`}>{category}</Link>
    <span>{">"}</span>
    <Link className="lg:hover:text-cyan-500" href={`/${category}/${subcat}`}>{subcat}</Link>
    <span>{">"}</span>
...
const PriceDisplay = ({ filteredProduct, priceBeforeDiscount }) => (
  <div className="font-bold mt-[10px]">
    <span className="text-[30px]" itemProp="offers" itemScope itemType="http://schema.org/Offer">
      ₹{parseInt(filteredProduct.price, 10).toLocaleString("en-IN")}
    </span>
    {priceBeforeDiscount && (
      <>
        <span className="line-through text-[#878787] ml-3" itemProp="priceBeforeDiscount">
          {parseInt(priceBeforeDiscount, 10).toLocaleString("en-IN")}
        </span>
        <span className="text-[#388e3c] ml-3" itemProp="discount" itemScope itemType="http://schema.org/Discount">
